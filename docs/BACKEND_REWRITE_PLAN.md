...
What is *not* covered by that: the follow-ups written up in **§10**, along with what to run on a fresh machine. Nothing there is architectural. §10's one real defect — the schema's three competing sources of truth, one of which was missing a table — and the three smaller items that followed have since been closed (schema idempotency, coverage floors, refresh-token rotation). The entries below record how; the frontend TypeScript adoption that followed
is also closed — see `docs/NEXT_STEPS.md`.
...
- *Phase 7 (optional stretch)*: **done, all four items.** §7's last item listed four things that were explicitly "not required for done". Three were built in Phase 7; the fourth (refresh-token rotation) was assessed, deferred past the rewrite's scope freeze, and later built end to end — see the rotation entry below and `docs/NEXT_STEPS.md`.

  **Pluggable password policies.** `PasswordPolicy` and its two implementations live in `domain/policies/`, not `infrastructure/` — "at least eight characters" is a business rule, not an integration. `LengthPasswordPolicy` is the default and its `requirement` string reproduces the message `authValidators.ts` used to hard-code, so the 400 body is byte-for-byte unchanged; `StrongPasswordPolicy` adds character variety behind `PASSWORD_POLICY=strong`. `violations()` returns every reason rather than the first, so someone fixing a password learns all of it in one round trip. The container's `selectPasswordPolicy` is the only `switch` — the same instance is handed to `AuthService`, the auth router, and the OpenAPI builder, which is what makes the documented rule necessarily the enforced one. Turning the strong policy on does not migrate existing accounts: stored hashes keep verifying, and only new or changed passwords are held to it.

  **Request-scoped correlation IDs.** A `RequestContext` port over `AsyncLocalStorage`, and `CorrelatingLogger` — a Decorator that implements `Logger` and wraps a `Logger`, applied once at the composition root. That last detail is the whole point: `AuthService` still asks for a `Logger` and still calls `logger.warn(message, meta)`, and every consumer written before the feature existed got correlation IDs without changing a line. It also covers log lines the application never writes itself, since the error middleware, the event bus, and `PostgresConnection` all take the same decorated instance. The middleware is mounted first — ahead of helmet and the rate limiter — so the ID is on the response header even for replies the app never consciously produces (a 429, a helmet rejection, a 404), which is what makes "quote the `X-Request-Id`" a workable bug report. An inbound ID is reused only if it matches `/^[A-Za-z0-9._:-]{1,128}$/`; that bound is security-relevant rather than cosmetic, because the value lands in log files, and permitting CR/LF would let a client forge whole log entries. The frontend's axios client already sent `X-Request-ID`, and its format passes the check, so traces join up across the two tiers without a frontend change.

  **OpenAPI at `/api-docs.json`.** 10 paths and 13 operations, assembled rather than hand-maintained: the status and priority enums come from the value objects, the password requirement from the active policy, and the response shapes from the DTOs via `PropertiesOf<T>` — a mapped type over `Required<T>` that forces the schema to name every field of the DTO and only its fields. Add a field to `TaskResponse` and `schemas.ts` stops compiling until it is documented; rename one and the stale key is rejected as an excess property. That is what "generated from the DTOs" actually buys here — not less typing, but documentation that cannot silently go stale, enforced by `npm run typecheck` in CI. Types are erased at runtime, so nothing can reflect over an interface to emit this automatically; pinning the keys is the strongest link available short of moving the API to a runtime schema library, which would mean replacing express-validator — a much larger change than this phase called for. Two drift tests run in opposite directions, because either alone leaves a way to be wrong: one probes every documented path to catch documenting an endpoint that does not exist, the other walks the mounted routers to catch shipping one that is undocumented. The document is built once at composition, not per request, since it depends only on configuration. Serving it publicly is deliberate: it describes the same endpoints the frontend bundle already reveals, carries no keys, and documents `/metrics` as existing without saying how to open it.

  **Refresh-token rotation: assessed during Phase 7, built afterward.** Phase 7 correctly refused to ship a backend-only half: a second token on login/register (§6 wire freeze), a frontend 401→refresh→retry path plus a second `secureStorage` credential (§8), and a `refresh_tokens` table (§8 schema freeze) were all out of scope then. Once those freezes lifted (`docs/NEXT_STEPS.md` Option A), it was built end to end: idempotent `refresh_tokens` in `schema.sql`, `RefreshTokenService` with HMAC via `JWT_REFRESH_SECRET` and family reuse detection inside `PostgresConnection.transaction()`, `POST /api/auth/refresh`, short-lived access JWTs (`JWT_EXPIRE` default `15m`), and the axios interceptor / `secureStorage` pair on the frontend. That also closes §10 items 3 and 4.

  Backend test count after the follow-up work is higher than Phase 7's 252/31; see `docs/NEXT_STEPS.md` for the current baseline.
...
**Pre-existing bug found while migrating, deliberately left alone at the time — later fixed:** `TaskModal` sends `due_date: ''` when the date field is empty, and `body('due_date').optional()` in express-validator only skips `undefined`, so `''` reached `isISO8601()` and failed. Creating a task without a due date from the UI therefore returned 400 `Invalid date format`. Closed afterward via `optional({ values: 'falsy' })` plus unit/integration tests (`docs/NEXT_STEPS.md` item 1).
...
7. **Optional stretch, not required for "done"** — pluggable password-policy strategies, refresh-token rotation as its own service, request-scoped correlation IDs in logs, OpenAPI generation from the DTOs. *(All four done: three in Phase 7; refresh-token rotation afterward as a deliberate wire-format + frontend + schema change — see the Phase 7 entry above and `docs/NEXT_STEPS.md`.)*
...
**2. No coverage thresholds. Closed.** `app/backend/vitest.config.js` now sets floors a point or two under the measured baseline (statements 93 / branches 81 / functions 95 / lines 93) so CI fails on backsliding without demanding new tests on the day the floors landed.

**3. `PostgresConnection.transaction()` has no production caller. Closed.** Refresh-token rotation is the first production caller: revoke-old + insert-new runs inside `PostgresRefreshTokenRepository.rotate`.

**4. `JWT_REFRESH_SECRET` is dead config. Closed.** Consumed by `RefreshTokenService` (HMAC of opaque refresh tokens). Required and distinct from `JWT_SECRET` in production.
...
`npm run lint` does **not** check formatting — ESLint and Prettier are separate here, so a file can pass lint and still be unformatted. Phase 7 shipped four such files before they were caught. Current counts live in `docs/NEXT_STEPS.md` (they move as follow-up work lands). The tests need a reachable Postgres; the integration and security suites are not mocked.
