...
- *Phase 7 (optional stretch)*: **done, three items of four.** §7's last item listed four things that were explicitly "not required for done". Three are built; the fourth was assessed and deliberately not built, for reasons recorded below.
  ...
  **Refresh-token rotation: assessed, not built.** It is the one item of the four that cannot be done as a backend change. It collides with three separate boundaries this plan set for itself: the login and register responses would have to carry a second token (§6 — "any other wire-format change should be treated as out of scope unless it's specifically raised and agreed on first"), the axios 401 interceptor would have to attempt a refresh and retry instead of clearing the session and redirecting, and `secureStorage` would have to hold a second credential (§8 — "no component, hook, or API-client changes"), and rotation with reuse detection needs a `refresh_tokens` table (§8 — "tables/columns stay as-is"). Building only the backend half yields an endpoint nothing calls: dead code that still has to be tested, documented, and kept working, and that adds a persisted long-lived credential to the attack surface for no benefit until the frontend catches up. The honest sequencing is that this is a frontend-and-backend feature, and it should be picked up when the frontend is in scope — at which point the access-token lifetime is the thing worth revisiting, since `JWT_EXPIRE` defaults to `7d` and `TokenService` independently caps token age at the same seven days. Worth knowing for whoever does pick it up: `JWT_REFRESH_SECRET` is already read into `Config.jwt.refreshSecret` and provisioned in `render.yaml` and `.env.example.secure`, but nothing consumes it. It is a placeholder for this feature, left in place rather than removed so the deployment config does not churn twice.

  252 tests across 31 files pass (up from 216 across 27); typecheck, lint, build, and Prettier are clean.
...
7. **Optional stretch, not required for "done"** — pluggable password-policy strategies, refresh-token rotation as its own service, request-scoped correlation IDs in logs, OpenAPI generation from the DTOs. *(Done as Phase 7, except refresh-token rotation. Three of these are backend-only and were built; rotation is not — it needs a wire-format change, a frontend change, and a new table, all three of which §6 and §8 put out of scope, and the backend half alone would be an endpoint nothing calls. Assessment recorded in the Phase 7 entry above.)*
